...
def get_flat_sensors(
    ...
    """
    Identify zero-variance (constant) sensors only.
    Used as a first pass to drop completely useless sensors before RUL
...
def select_sensors_by_rul_correlation(
    ...
    """
    Select sensors by their average absolute correlation with RUL
    across ALL subsets.

    Why this is better than variance-based selection:
        Variance tells us a sensor changes — but not whether it changes
        WITH degradation. A noisy constant sensor has variance but no
        correlation with RUL. This method directly asks:
        "does this sensor predict RUL?"

    Only keeps sensors that are predictive in ALL subsets — ensures
    the model learns universal degradation patterns, not subset-specific
    noise.

    Args:
        train_all:       Full concatenated training DataFrame with 'rul' column
        subsets:         List of subset names e.g. ['FD001','FD002','FD003','FD004']
        min_correlation: Minimum avg absolute correlation to keep sensor

    Returns:
        List of sensor column names to keep
    """
    sensor_cols = [c for c in train_all.columns if c in ALL_SENSOR_COLS]

    # Compute per-subset correlation with RUL
    ...
    # Average correlation across all subsets
    avg_corrs = {}
    for col in sensor_cols:
    ...
    print(f"      Sensor correlations with RUL (avg across subsets):")
    ...
    print(f"      Keeping {len(selected)} sensors (corr >= {min_correlation}): {sorted(selected)}")
...
def preprocess(
    ...
    # ── Step 2: Select sensors by RUL correlation ─────────────────────────
    print("[2/9] Selecting sensors by RUL correlation...")

    # Drop zero-variance sensors first (completely constant, useless)
    flat_sensors = get_flat_sensors(train_all)
    if flat_sensors:
        print(f"      Dropping zero-variance sensors: {flat_sensors}")
        train_all = drop_flat_sensors(train_all, flat_sensors)
        test_all  = drop_flat_sensors(test_all,  flat_sensors)

    # Compute RUL temporarily so we can correlate sensors with it
    train_for_sel = compute_rul(train_all, rul_cap)

    # Select sensors by avg absolute correlation with RUL across all subsets
    ...
    X_tr, s_tr, r_tr, c_tr = build_windows(
        train_df, sensor_cols, window_size, stride=stride
    )
